...
The user observed T-shirt designs not brought to the active event in SKU POS. The event opening saves zero placeholder rows for all catalog SKUs, and the prior POS filter accepted every matching variantId even when all sizes of a design had zero event inventory. For an active Session with an opening, POS now shows a T-shirt design only if at least one matching SKU had positive opening, positive Restock or opening correction, or an exact SKU sale. Zero-quantity sizes in such a design remain visible when they were already POS candidates. Without an event opening, the existing catalog visibility remains unchanged. The selection is presentation-only and does not change stock or sales. A design with only automatic zero placeholders is not distinguishable from an intentionally registered zero-only design in the current Session schema; this implementation hides such a design until a positive event adjustment is recorded. Focused tests cover the display filter. No production Session was mutated.
