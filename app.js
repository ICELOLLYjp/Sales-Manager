...
async function renderMorePage(sequence) {
  ...
  try {
    ...
    const registeredTshirtVariants =
      allVariants.filter(
        item =>
          item.category ===
          "tshirt"
    ...
    const registeredTshirtIds =
      new Set(
        registeredTshirtVariants
          .map(
            item =>
              item.variantId ||
              item.id
    ...
    const registeredNotInPinkoi =
      registeredTshirtVariants
        .filter(
          item =>
            !pinkoiVariantIds.has(
              item.variantId ||
              item.id
            )
        );

    const pinkoiNotRegistered =
      (
        pinkoiTshirtCatalog
          ?.variants ||
        []
        ...
        ${
          pinkoiTshirtCatalog.error
            ...
            : `
              ...
              <div class="list-row">
                <span>登録済みTシャツSKU</span>
                <strong>${registeredTshirtVariants.length}</strong>
              </div>

              <div class="list-row">
                <span>Pinkoiに存在しない登録SKU</span>
                <strong>${registeredNotInPinkoi.length}</strong>
              </div>

              <div class="list-row">
                <span>Pinkoiから未登録のSKU</span>
                <strong>${pinkoiNotRegistered.length}</strong>
              </div>

              ${
                registeredNotInPinkoi.length
                  ? `
                    <details style="margin-top:12px;">
                      <summary
                        ...
                        "
                      >
                        Pinkoiに存在しない登録SKUの詳細
                      </summary>

                      <div style="margin-top:8px;">
                        ${registeredNotInPinkoi
                          .map(
                            item => `
                              <div
                                style="
              ...
              ${
                pinkoiNotRegistered.length
                  ? `
                    <div
                      class="warning"
                      style="margin-top:12px;"
                    >
                      Pinkoi診断では同期可能ですが、productVariantsへ未登録のSKUが
                      ${pinkoiNotRegistered.length}
                      件あります。
                    </div>
                  `
