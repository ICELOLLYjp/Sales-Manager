...
function findHistorySections() {
  return Array.from(document.querySelectorAll(".card-title"))
    ...
    .map(element => ({ title: element, section: element.parentElement }))
    .filter(item => item.section);
}

function enhanceHistorySection({ title, section }) {
  if (section.querySelector(`.${CONTROL_CLASS}`)) return;
  ...
  controls.innerHTML = `
    <input class="transaction-history-search-input" type="search"
      placeholder="Transaction ID・時刻などで検索" autocomplete="off"
      style="width:100%;min-height:44px;padding:0 12px;border:1px solid #deded9;border-radius:12px;background:#fff;">
    <input class="transaction-history-amount-filter" type="number"
    ...
    <div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;">
      <select class="transaction-history-payment-filter"
        style="width:100%;min-height:40px;padding:0 9px;border:1px solid #deded9;border-radius:10px;background:#fff;">
        <option value="all">支払方法：すべて</option>
        <option value="stripe">Stripe</option>
        <option value="other">Stripe以外</option>
      </select>
      <select class="transaction-history-status-filter"
        style="width:100%;min-height:40px;padding:0 9px;border:1px solid #deded9;border-radius:10px;background:#fff;">
    ...
    <div class="transaction-history-search-count muted" style="min-height:16px;font-size:11px;"></div>
  `;
  title.insertAdjacentElement("afterend", controls);

  const searchInput = controls.querySelector(".transaction-history-search-input");
  const amountFilter = controls.querySelector(".transaction-history-amount-filter");
  const paymentFilter = controls.querySelector(".transaction-history-payment-filter");
  const statusFilter = controls.querySelector(".transaction-history-status-filter");
  const count = controls.querySelector(".transaction-history-search-count");
  ...
  function applyFilter() {
    ...
    transactionRows.forEach(row => {
      ...
      const visible = searchMatches && amountMatches && paymentMatches && statusMatches;
      row.style.display = visible ? "" : "none";
      if (visible) visibleCount += 1;
    });

    if (count) count.textContent = `${visibleCount} / ${transactionRows.length} 件`;
  }
...
if (view) {
  const observer = new MutationObserver(enhanceTransactionHistory);
